...
export const Header: React.FC<HeaderProps> = ({ currentRoute, onNavigate, selectedCourseSlug }) => {
  ...
  const navBtn = (active: boolean) =>
    `relative flex items-center gap-1.5 px-3 py-2 text-xs xl:text-sm font-semibold rounded-xl transition-all duration-200 cursor-pointer
     ${active
       ? 'text-[#78122B] bg-[#78122B]/8 font-bold'
       : 'text-[#4A3B3D] hover:text-[#78122B] hover:bg-[#78122B]/6'}`;
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 sm:h-[70px] items-center justify-between gap-3">
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5" aria-label="Main navigation">
              ...
              <AnimatePresence>
                {activeDropdown === 'courses' && (
                  <motion.div
                    ...
                    className="absolute left-0 top-full mt-2 w-[380px] rounded-2xl border border-[#E8DDD9] bg-white p-3 shadow-xl z-50 space-y-2"
                    id="dropdownCoursesPanel"
                  >
                    <button
                      onClick={() => go('women')}
                      className={`w-full text-left p-3 rounded-xl flex items-start gap-3 transition-all cursor-pointer group/c ${
                        currentRoute === 'women' ? 'bg-[#78122B] text-white' : 'bg-[#FAF8F5] hover:bg-[#F9E8EC] text-[#23181A]'
                      }`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs mt-0.5 line-clamp-1 ${currentRoute === 'women' ? 'text-white/75' : 'text-[#8C7A7E]'}`}>
                          Seerah, Tajweed 1-on-1, Noorani Qaida & Pre Diploma
                        </p>
                      </div>
                    </button>
                    ...
                    <button
                      onClick={() => go('kids')}
                      className={`w-full text-left p-3 rounded-xl flex items-start gap-3 transition-all cursor-pointer group/c ${
                        currentRoute === 'kids' ? 'bg-[#78122B] text-white' : 'bg-[#FAF8F5] hover:bg-[#F9E8EC] text-[#23181A]'
                      }`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs mt-0.5 line-clamp-1 ${currentRoute === 'kids' ? 'text-white/75' : 'text-[#8C7A7E]'}`}>
              ...
              <AnimatePresence>
                {activeDropdown === 'sacred' && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 6, scale: 0.97 }}
                    transition={{ duration: 0.16 }}
                    className="absolute left-0 top-full mt-2 w-64 rounded-2xl border border-[#E8DDD9] bg-white p-2.5 shadow-xl z-50 space-y-1"
            ...
            {/* 3. Direct Menu Links */}
            <button onClick={() => go('freeCourses')} className={navBtn(currentRoute === 'freeCourses')} id="navBtnFreeCourses">
              Free Courses
            </button>
            <button onClick={() => go('scholarship')} className={navBtn(currentRoute === 'scholarship')} id="navBtnScholarship">
              Scholarship
            </button>
            <button onClick={() => go('about')} className={navBtn(currentRoute === 'about')} id="navBtnAbout">
              About
            </button>
            <button onClick={() => go('faq')} className={navBtn(currentRoute === 'faq')} id="navBtnFaq">
              FAQ
            </button>
            <button onClick={() => go('contact')} className={navBtn(currentRoute === 'contact')} id="navBtnContact">
              Contact
            </button>
          </nav>

          {/* ── Desktop Right Actions (Single Search + Single Register) ────── */}
          <div className="hidden lg:flex items-center gap-2 shrink-0">
            <button
              onClick={() => setShowSearch(s => !s)}
              className="p-2 rounded-xl text-[#8C7A7E] hover:text-[#78122B] hover:bg-[#78122B]/6 transition-all cursor-pointer"
              aria-label="Search"
              id="navSearchBtn"
            >
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#78122B] text-white text-xs xl:text-sm font-semibold hover:bg-[#630E23] transition-all shadow-xs cursor-pointer active:scale-95"
              id="navRegisterBtn"
            >
              Register
            </a>
          ...
          {/* ── Mobile Right Actions (Search + Register + Hamburger Toggle) ── */}
      ...
      {/* ── Mobile Menu Drawer (Rendered via Portal directly to document.body) ── */}
