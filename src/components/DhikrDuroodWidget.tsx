...
import { Sparkles, RotateCcw, Volume2, VolumeX, X, CheckCircle2, Heart, Info, BookOpen, Flame, Award, Instagram } from 'lucide-react';

// Web Audio API soft chime helper
...
// 1-Minute Dhikr Routine item definitions
interface DhikrItem {
  id: string;
  arabic: string;
...
export const DhikrDuroodWidget: React.FC = () => {
  ...
  // Total Dhikr count completed across all steps
  ...
  const handleIncrementDhikr = () => {
    ...
    const currentStepCount = dhikrCounts[step.id] || 0;

    if (currentStepCount < step.target) {
      const newCount = currentStepCount + 1;
      const updatedCounts = { ...dhikrCounts, [step.id]: newCount };
      setDhikrCounts(updatedCounts);

      // Check if current step reached target
      if (newCount === step.target) {
        if (currentStepIndex < dhikrRoutine.length - 1) {
          // Play higher chime for step transition
  ...
  const handleResetDurood = () => {
    setDuroodCount(0);
  };

  const currentDhikrStep = dhikrRoutine[currentStepIndex];
  const stepCount = dhikrCounts[currentDhikrStep.id] || 0;
  const stepProgress = Math.min(100, Math.round((stepCount / currentDhikrStep.target) * 100));
  const totalDhikrProgress = Math.min(100, Math.round((totalDhikrCount / totalDhikrTarget) * 100));
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md select-none">
            ...
            {activeModal === 'dhikr' && (
              <motion.div
                ...
                className="relative w-full max-w-lg bg-[#1B1214] border border-[#D4AF37]/40 rounded-3xl shadow-2xl overflow-hidden z-10 text-white flex flex-col max-h-[90vh]"
                id="dhikr-counter-modal"
              >
                {/* Header */}
                <div className="p-5 border-b border-[#D4AF37]/20 flex items-center justify-between bg-[#23181A]">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-2xl bg-[#78122B]/50 border border-[#D4AF37]/30 text-[#F3D797]">
                      📿
                    </div>
                    <div>
                      <h3 className="serif-heading text-lg font-bold text-[#F3D797]">
                        1-Minute Daily Dhikr Routine
                      </h3>
                      <p className="text-[11px] text-white/70">
                        Quick 60-Second Sunnah Tasbeeh
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title={soundEnabled ? 'Disable Audio Tone' : 'Enable Audio Tone'}
                      id="dhikr-sound-toggle-btn"
                    >
                ...
                {/* Overall Routine Progress Steps Pills */}
                <div className="p-3 bg-[#140D0F] border-b border-white/5 overflow-x-auto no-scrollbar flex items-center gap-2">
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                          isCurrent
                            ? 'bg-[#D4AF37] text-[#1B1214] font-bold shadow-md scale-105'
                            : isDone
                            ? 'bg-[#125A3A]/60 text-emerald-200 border border-emerald-500/30'
                            : 'bg-white/5 text-white/60 hover:bg-white/10'
                        }`}
                        id={`dhikr-step-pill-${item.id}`}
                      >
                        {isDone ? (
                ...
                <div className="p-6 overflow-y-auto flex-grow flex flex-col items-center justify-between text-center space-y-6">
                  ...
                  {dhikrCompleted ? (
                    <motion.div
                      ...
                      className="py-6 space-y-4 text-center my-auto"
                    >
                      <div className="w-16 h-16 rounded-full bg-[#125A3A] text-emerald-200 flex items-center justify-center mx-auto text-2xl shadow-xl border border-emerald-400/40">
                        ✨
                      </div>
                      <h4 className="serif-heading text-2xl font-bold text-[#F3D797]">
                        MashaAllah! Routine Completed
                      </h4>
                      <p className="text-xs text-white/80 max-w-xs mx-auto leading-relaxed">
                        You completed all 120 recitations of the 1-Minute Daily Dhikr. May Allah accept your remembrance and illuminate your heart.
                      </p>

                      <div className="p-4 rounded-2xl bg-white/5 border border-[#D4AF37]/20 text-xs space-y-1.5 text-left max-w-sm mx-auto">
                        <p className="text-[#F3D797] font-semibold flex items-center gap-1">
                          <Flame className="w-3.5 h-3.5" /> 1-Minute Checklist Completed:
                        </p>
                      ...
                      <button
                        ...
                        className="px-6 py-3 rounded-2xl bg-[#D4AF37] text-[#1B1214] font-bold text-xs hover:bg-[#F3D797] transition-all shadow-lg flex items-center gap-2 mx-auto cursor-pointer"
                        id="dhikr-restart-btn"
                      >
                    ...
                    <>
                      ...
                      <div className="w-full space-y-3 pt-2">
                        <div className="text-3xl sm:text-4xl font-serif text-[#F3D797] font-bold tracking-wide leading-relaxed">
                          {currentDhikrStep.arabic}
                        </div>
                        <div className="text-lg font-bold text-white tracking-wide">
                          {currentDhikrStep.transliteration}
                        </div>
                        <div className="text-xs text-[#D4AF37]/80 italic">
                          "{currentDhikrStep.meaning}"
                        </div>
                      </div>
                      ...
                      <div className="py-2 flex flex-col items-center justify-center relative">
                        <motion.button
                          ...
                          className="relative w-40 h-40 sm:w-44 sm:h-44 rounded-full bg-gradient-to-b from-[#78122B] to-[#420A17] border-4 border-[#D4AF37]/60 shadow-[0_0_40px_rgba(212,175,55,0.25)] flex flex-col items-center justify-center cursor-pointer group select-none active:brightness-125 transition-all"
                          id="dhikr-main-tap-btn"
                        >
                          ...
                          <svg className="absolute inset-0 w-full h-full -rotate-90 p-1" viewBox="0 0 100 100">
                            ...
                            <circle
                              ...
                              cy="50"
                              r="46"
                              className="stroke-[#F3D797] transition-all duration-300"
                              strokeWidth="5"
                              strokeDasharray={289}
                          ...
                          <span className="text-4xl sm:text-5xl font-black text-[#F3D797] my-1 font-mono">
                            {stepCount}
                          </span>
                          <span className="text-[11px] text-white/70 font-semibold">
                            Target: {currentDhikrStep.target}
                          </span>
                        </motion.button>

                        <p className="text-[10px] text-white/50 mt-3">
                          Tap anywhere inside the circle to count
                        </p>
                      </div>
                      ...
                      <div className="w-full space-y-1.5 bg-white/5 p-3 rounded-2xl border border-white/10">
                        ...
                        <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                          <div
                            className="bg-gradient-to-r from-[#D4AF37] to-[#F3D797] h-full transition-all duration-300"
                            style={{ width: `${totalDhikrProgress}%` }}
                          />
                        </div>
                      </div>
                    </>
                ...
                <div className="p-4 border-t border-white/10 bg-[#23181A] flex items-center justify-between text-xs text-white/60">
                  ...
                  <span className="text-[11px] text-[#F3D797]/70 italic">
                    SubhanAllah &bull; Alhamdulillah &bull; Allahu Akbar
                  </span>
                </div>
            ...
            {activeModal === 'durood' && (
              <motion.div
                ...
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                className="relative w-full max-w-lg bg-[#082819] border border-[#D4AF37]/40 rounded-3xl shadow-2xl overflow-hidden z-10 text-white flex flex-col max-h-[90vh]"
                id="durood-counter-modal"
              >
                {/* Header */}
                <div className="p-5 border-b border-[#D4AF37]/20 flex items-center justify-between bg-[#0B3C26]">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-2xl bg-[#125A3A] border border-[#D4AF37]/30 text-[#F3D797]">
                      ✨
                    </div>
                    <div>
                      <h3 className="serif-heading text-lg font-bold text-[#F3D797]">
                        Durood for Nabi-al-Ummi
                      </h3>
                      <p className="text-[11px] text-emerald-200/80">
                        The Unlettered Prophet ﷺ
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setShowVirtuesTab(!showVirtuesTab)}
                      className={`p-2 rounded-xl transition-colors cursor-pointer text-xs font-semibold flex items-center gap-1 ${
                        showVirtuesTab ? 'bg-[#D4AF37] text-[#082819]' : 'bg-white/10 hover:bg-white/20 text-[#F3D797]'
                      }`}
                      title="View Virtues & Benefits"
                      id="durood-virtues-toggle-btn"
                    >
                ...
                {/* Sub Navigation / Target Selector */}
                <div className="p-3 bg-[#051C12] border-b border-white/5 flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-1.5">
                    {[10, 33, 80, 100].map((t) => (
                      <button
                        key={t}
                        onClick={() => setDuroodTarget(t)}
                        className={`px-2.5 py-1 rounded-lg font-bold text-xs transition-all cursor-pointer ${
                          duroodTarget === t
                            ? 'bg-[#D4AF37] text-[#082819] shadow-sm'
                            : 'bg-white/5 text-white/60 hover:bg-white/10'
                        }`}
                        id={`durood-target-btn-${t}`}
                      >
                        {t === 80 ? '80 (Friday Sunnah)' : t}
                      </button>
                    ))}
                  </div>
                </div>

                {/* Content View: Virtues vs Interactive Counter */}
                <div className="p-6 overflow-y-auto flex-grow flex flex-col items-center justify-between text-center space-y-6">
                  ...
                  {showVirtuesTab ? (
                    <motion.div
                      ...
                      className="w-full text-left space-y-4 my-auto"
                    >
                      <div className="p-4 rounded-2xl bg-white/5 border border-[#D4AF37]/30 space-y-2">
                        <h4 className="text-sm font-bold text-[#F3D797] flex items-center gap-2">
                          <Award className="w-4 h-4 text-[#D4AF37]" /> Friday Virtue (80 Times After Asr)
                        </h4>
                        <p className="text-xs text-emerald-100/90 leading-relaxed">
                          Islamic scholars highlight that reciting this specific Durood 80 times after the Asr prayer on Friday leads to the forgiveness of 80 years of sins and writes 80 years of worship in your book of deeds.
                        </p>
                      </div>

                      <div className="p-4 rounded-2xl bg-white/5 border border-[#D4AF37]/30 space-y-2">
                        <h4 className="text-sm font-bold text-[#F3D797] flex items-center gap-2">
                          <BookOpen className="w-4 h-4 text-[#D4AF37]" /> Ease of Recitation
                        </h4>
                        <p className="text-xs text-emerald-100/90 leading-relaxed">
                          It is short, easy to memorize, and perfect for abundant daily recitation (dhikr) throughout your day.
                        </p>
                      </div>

                      <div className="p-4 rounded-2xl bg-white/5 border border-[#D4AF37]/30 space-y-2">
                        <h4 className="text-sm font-bold text-[#F3D797] flex items-center gap-2">
                          <Heart className="w-4 h-4 text-[#D4AF37]" /> Deep Connection
                        </h4>
                        <p className="text-xs text-emerald-100/90 leading-relaxed">
                          It specifically honors the Prophet’s attribute as "Al-Ummi" (the unlettered), as explicitly mentioned in the Holy Qur'an.
                        </p>
                      </div>

                      <button
                        onClick={() => setShowVirtuesTab(false)}
                        className="w-full py-3 rounded-2xl bg-[#D4AF37] text-[#082819] font-bold text-xs hover:bg-[#F3D797] transition-all cursor-pointer text-center"
                      >
                        Back to Counter
                      </button>
                    </motion.div>
                    ...
                    <>
                      ...
                      <div className="w-full space-y-3 pt-1">
                        <div className="text-2xl sm:text-3xl font-serif text-[#F3D797] font-bold tracking-wide leading-relaxed dir-rtl">
                          اَللّٰهُمَّ صَلِّ عَلٰى مُحَمَّدٍ النَّبِيِّ الْأُمِّيِّ وَعَلٰى آلِهِ وَسَلِّمْ
                        </div>
                        <div className="text-xs sm:text-sm font-semibold text-emerald-100/90 leading-snug">
                          Allahumma salli ‘ala Muhammadin-in-Nabiyyil-Ummiyyi wa ‘ala aalihi wa sallim.
                        </div>
                        <div className="text-[11px] text-[#D4AF37]/80 italic max-w-sm mx-auto">
                          "O Allah, send blessings upon Muhammad, the Unlettered Prophet, and upon his family, and grant them peace."
                        </div>
                      </div>
                      ...
                      <div className="py-2 flex flex-col items-center justify-center relative">
                        <motion.button
                          onClick={handleIncrementDurood}
                          whileTap={{ scale: 0.92 }}
                          whileHover={{ scale: 1.03 }}
                          className="relative w-40 h-40 sm:w-44 sm:h-44 rounded-full bg-gradient-to-b from-[#125A3A] to-[#082819] border-4 border-[#D4AF37]/60 shadow-[0_0_40px_rgba(212,175,55,0.25)] flex flex-col items-center justify-center cursor-pointer group select-none active:brightness-125 transition-all"
                          id="durood-main-tap-btn"
                        >
                          ...
                          <svg className="absolute inset-0 w-full h-full -rotate-90 p-1" viewBox="0 0 100 100">
                            ...
                            <circle
                              ...
                              strokeWidth="5"
                              strokeDasharray={289}
                              strokeDashoffset={289 - (289 * duroodProgress) / 100}
                              strokeLinecap="round"
                              fill="transparent"
                            />
                          </svg>

                          <span className="text-[10px] uppercase tracking-widest text-emerald-200/70 font-medium">
                            RECITED DUROOD
                          </span>
                          <span className="text-4xl sm:text-5xl font-black text-[#F3D797] my-1 font-mono">
                            {duroodCount}
                          </span>
                          <span className="text-[11px] text-emerald-100/70 font-semibold">
                            Target: {duroodTarget}
                          </span>
                        </motion.button>

                        <p className="text-[10px] text-emerald-200/50 mt-3">
                          Tap to count each Durood Shareef
                        </p>
                      </div>
                ...
                <div className="p-4 border-t border-white/10 bg-[#0B3C26] flex items-center justify-between text-xs text-emerald-100/70">
                  <button
                    onClick={handleResetDurood}
                    className="flex items-center gap-1.5 text-emerald-200/70 hover:text-white transition-colors cursor-pointer"
                    id="durood-reset-btn"
                  >
                  ...
                  <button
                    onClick={() => setShowVirtuesTab(!showVirtuesTab)}
                    className="text-[#F3D797] hover:underline cursor-pointer text-xs font-semibold"
                  >
                    {showVirtuesTab ? 'Back to Counter' : 'Read Friday Virtues →'}
                  </button>
                </div>
