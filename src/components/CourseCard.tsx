...
export const CourseCard: React.FC<CourseCardProps> = ({ course, onSelect }) => {
  ...
  return (
    <motion.div 
      ...
      className="group flex flex-col h-full rounded-2xl border border-[#D4AF37]/20 bg-panel-dark overflow-hidden shadow-lg transition-colors duration-300 hover:border-[#D4AF37] hover:shadow-2xl"
      id={`course-card-${course.slug}`}
    >
      {/* Course Thumbnail Image */}
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-bg-deep border-b border-[#D4AF37]/20">
        {course.badge && (
          <div 
            className="absolute top-3 left-3 z-10 rounded-full bg-[#D4AF37] px-3 py-1 text-[11px] font-extrabold tracking-wider text-[#480117] uppercase shadow-sm border border-[#D4AF37]/20 backdrop-blur-sm"
      ...
      {/* Content Area */}
      <div className="flex flex-col flex-1 p-6 space-y-4 bg-panel-dark">
        <div className="space-y-1.5">
          ...
          <h3 
            className="serif-heading text-xl font-bold leading-snug text-white group-hover:text-[#D4AF37] transition-colors duration-300"
            id={`course-title-${course.slug}`}
          >
            {course.title}
          </h3>
          <p className="text-sm italic font-medium text-[#EAD5D8]/90 line-clamp-2">
            "{course.hook}"
          </p>
        </div>

        {/* Quick Facts Grid */}
        <div className="grid grid-cols-2 gap-3.5 py-3 border-y border-white/10 text-xs text-[#EAD5D8]/80">
          <div className="flex items-center space-x-2">
            <Clock className="w-4 h-4 text-[#D4AF37]/80 shrink-0" />
            <span className="truncate">{course.duration}</span>
          </div>
          <div className="flex items-center space-x-2">
            <Layers className="w-4 h-4 text-[#D4AF37]/80 shrink-0" />
            <span className="truncate">{course.courseDetails['Format'] || 'Google Meet'}</span>
          </div>
          <div className="col-span-2 flex items-center space-x-2">
            <Calendar className="w-4 h-4 text-[#D4AF37]/80 shrink-0" />
            <span className="truncate font-semibold text-[#D4AF37]">
              {course.price} <span className="text-[10px] font-normal text-[#EAD5D8]/80">/{course.priceDetail}</span>
            </span>
          </div>
        </div>
        ...
        <div className="mt-auto pt-2 flex items-center justify-between">
          <button
            ...
            className="inline-flex items-center space-x-1.5 text-xs font-semibold uppercase tracking-wider text-[#D4AF37] hover:text-white group/btn transition-colors duration-300 min-h-[44px] md:min-h-0 py-2.5 md:py-1"
          ...
          <a
            ...
            className="inline-flex h-11 md:h-9 items-center justify-center rounded-xl bg-[#8E4B59] hover:bg-[#743C47] border border-[#D4AF37]/20 px-3.5 text-xs font-semibold text-white transition-all duration-300 hover:shadow-md"
